# Imagen base (compartida entre todos los nodos)
...
# Disco OS por nodo, clonado desde la base con el tamaño de var.diskSize
resource "libvirt_volume" "os_image" {
  count          = var.vm_count
  name           = "${var.hostname_base}${count.index + 1}-os_image"
  pool           = "pool"
...
data "template_file" "user_data" {
  ...
  vars = {
    hostname   = "${var.hostname_base}${count.index + 1}"
